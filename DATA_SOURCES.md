...
|---|---|---|---|---|
| **SEC EDGAR XBRL** done | All US filer fundamentals, `filed` dates, restatements | none | yes, native | The only free point-in-time fundamentals that exist |
| **SEC `frames` API** | One concept, *all companies*, one period, cross-section in a single call | none | yes | Turns EDGAR from company-lookup into a factor engine |
| **Open Source Asset Pricing** (Chen-Zimmermann) | 200+ published anomaly signals, reproduced, with code | none | yes | Ground truth for the whole "verify published claims" thesis |
| **Ken French Data Library** | FF3/FF5, UMD, industry portfolios, breakpoints, international | none | yes | The benchmark every factor claim is scored against |
| **JKP Global Factor Data** (Jensen-Kelly-Pedersen) | 150+ factors, 93 countries, cluster-level | free key | yes | Out-of-sample test bed the free world has no access to otherwise |
| **FRED** (St. Louis Fed) | 800k+ macro series, rates, inflation, credit, sentiment | free key | yes, **vintages via ALFRED** | ALFRED gives real-time macro vintages. Almost nobody uses this and almost everyone should |
| **FINRA short sale volume** | Daily short volume by symbol, per venue | none | yes | Free, daily, genuinely predictive, painfully unglued |
| **CFTC Commitments of Traders** | Weekly futures positioning by trader class | none | yes | The classic free positioning dataset |
...
|---|---|---|---|
| **SEC 8-K** (EDGAR RSS, real time) | Material corporate events, filed to the minute | none | yes, exact, legally mandated |
| **EDGAR full-index + RSS** | Every filing as it lands, all filers | none | yes, exact |
| **GDELT 2.0** | Global news events, tone, actors, every 15 min since 2015 | none | yes, good |
| **Company IR press releases** (RSS/Atom) | Primary-source announcements, pre-spin | none | yes, exact |
| **Federal Reserve / FOMC / regional Fed RSS** | Statements, minutes, speeches, H.4.1 | none | yes, exact |
| **Central bank feeds** (ECB, BoE, BoJ, BoC) | Policy statements and speeches | none | yes, exact |
| **Treasury / BLS / BEA / Census release calendars** | Scheduled macro release times | none | yes, exact, lets you separate scheduled from surprise |
| **Wire and outlet RSS** (Reuters, AP, CNBC, Yahoo, Bloomberg headlines) | Headlines and links, unlimited | none | partial, publication time only |
| **Regulatory feeds** (FDA, FTC, DOJ, EPA, CFTC, FINRA actions) | Approvals, enforcement, recalls | none | yes, exact |
| **Nasdaq / NYSE notices** | Halts, delistings, listing deficiencies | none | yes, exact |
| **Wikipedia recent changes + pageviews** | Attention spikes per entity, per hour | none | yes, exact |
| **Google Trends** (`pytrends`) | Search interest, weekly/daily | unofficial | partial, rescaled, grey terms |
